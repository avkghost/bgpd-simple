...
static int pfx_eq(rib_prefix4_t a, rib_prefix4_t b){
  return a.pfx.s_addr == b.pfx.s_addr && a.plen == b.plen;
}
...
int rib4_find_entry(rib4_t* r, struct in_addr pfx, uint8_t plen){
  rib_prefix4_t k = {.pfx=pfx,.plen=plen};
  for(int i=0;i<r->entry_count;i++){
...
static int rib4_get_or_add_entry(rib4_t* r, struct in_addr pfx, uint8_t plen){
  int idx = rib4_find_entry(r, pfx, plen);
  if(idx >= 0) return idx;

  if(r->entry_count == r->entry_cap){
    int ncap = (r->entry_cap == 0) ? 64 : r->entry_cap * 2;
    rib_entry4_t* n = (rib_entry4_t*)realloc(r->entries, (size_t)ncap * sizeof(rib_entry4_t));
    if(!n) return -1;
    r->entries = n;
    r->entry_cap = ncap;
  }
...
void rib4_withdraw(rib4_t* r, bgp_peer_t* from, struct in_addr pfx, uint8_t plen){
  int e = rib4_find_entry(r, pfx, plen);
  if(e < 0) return;
  rib_entry4_t* re = &r->entries[e];
...
void rib4_add_or_replace(rib4_t* r, bgp_peer_t* from, struct in_addr pfx, uint8_t plen, const bgp_attrs_t* a){
  int e = rib4_get_or_add_entry(r, pfx, plen);
  if(e < 0) return;
  rib_entry4_t* re = &r->entries[e];
  ...
  if(re->path_count == re->path_cap){
    ...
    rib_path4_t* n = (rib_path4_t*)realloc(re->paths, (size_t)ncap * sizeof(rib_path4_t));
    if(!n) return;
    re->paths = n;
    re->path_cap = ncap;
  }
...
static bool path_is_ebgp(const rib_path4_t* p, uint32_t local_asn){
 ...
 */
static int better(const rib4_t* rib,
                  const rib_path4_t* a, const rib_path4_t* b){
  ...
  uint32_t rid_a = a->from ? a->from->remote_id.s_addr : rib->router_id.s_addr;
  uint32_t rid_b = b->from ? b->from->remote_id.s_addr : rib->router_id.s_addr;
  if(rid_a != rid_b) return (ntohl(rid_a) < ntohl(rid_b));

  return 0;
}

bool rib4_recompute_best(rib4_t* r, int entry_index){
  if(entry_index < 0 || entry_index >= r->entry_count) return false;
  rib_entry4_t* re = &r->entries[entry_index];
  int old = re->best_index;
  int best = -1;
